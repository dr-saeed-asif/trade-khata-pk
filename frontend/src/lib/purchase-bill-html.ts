...
const lineRows = (purchase: PurchaseRecord) =>
  purchase.lines
    .map((line, index) => {
...
const discountRow = (purchase: PurchaseRecord) =>
  purchase.discount > 0
    ? `<tr style="border-bottom:1px solid #e2e8f0;">
        <td style="padding:6px 8px;font-weight:500;">Discount</td>
        <td style="padding:6px 8px;text-align:right;">Rs ${purchase.discount.toFixed(2)}</td>
      </tr>`
    : ''

export const buildPurchaseBillDocument = (purchase: PurchaseRecord, title?: string) => {
  const logoUrl = resolveLogoUrl()
  const docTitle = escapeHtml(title ?? `${purchase.invoiceNo} - Purchase Bill`)
  const partyName = escapeHtml(purchase.party?.name ?? 'Walk-in Supplier')
  const partyPhone = purchase.party?.phone
    ? `<p style="margin:4px 0 0;font-size:11px;color:#64748b;">${escapeHtml(purchase.party.phone)}</p>`
    : ''
  const notesBlock = purchase.notes
    ? `<p style="margin:8px 0 0;font-size:11px;color:#64748b;"><strong>Notes:</strong> ${escapeHtml(purchase.notes)}</p>`
    : ''
...
<body>
  <div style="max-width:210mm;margin:0 auto;background:#fff;">
    ...
    ${invoiceBannerHtml('PURCHASE BILL', 'خریداری', PURPLE)}

    <table style="width:100%;border-collapse:collapse;margin-bottom:16px;">
      <tr>
        <td style="width:50%;vertical-align:top;">
          <p style="margin:0 0 4px;font-size:11px;font-weight:bold;color:${PURPLE};text-transform:uppercase;">Supplier</p>
        ...
        <td style="width:50%;vertical-align:top;text-align:right;font-size:12px;">
          <p style="margin:0;"><strong>Invoice No.:</strong> ${escapeHtml(purchase.invoiceNo)}</p>
          <p style="margin:6px 0 0;"><strong>Date:</strong> ${formatBillDate(purchase.purchaseDate)}</p>
        </td>
      </tr>
    </table>
      ...
      <tr>
        <td style="width:50%;vertical-align:top;padding-right:12px;">
          <div style="border:1px solid ${PURPLE};border-radius:4px;overflow:hidden;margin-bottom:12px;">
            <div style="background:${PURPLE};color:#fff;padding:6px 8px;font-size:11px;font-weight:bold;">Amount In Words</div>
            <p style="margin:0;padding:8px;font-size:11px;text-transform:capitalize;">${escapeHtml(amountToWords(purchase.total))}</p>
          </div>
            ...
            <table style="width:100%;border-collapse:collapse;font-size:11px;">
              <tbody>
                <tr style="background:${PURPLE_LIGHT};border-bottom:1px solid #e2e8f0;">
                  ...
                  <td style="padding:6px 8px;text-align:right;">Rs ${purchase.subtotal.toFixed(2)}</td>
                </tr>
                ${discountRow(purchase)}
                <tr style="border-bottom:1px solid #e2e8f0;font-weight:bold;">
                  <td style="padding:6px 8px;">Total</td>
                  <td style="padding:6px 8px;text-align:right;">Rs ${purchase.total.toFixed(2)}</td>
                </tr>
                <tr>
                  <td style="padding:6px 8px;font-weight:500;">Balance</td>
                  <td style="padding:6px 8px;text-align:right;font-weight:600;">Rs ${purchase.total.toFixed(2)}</td>
                </tr>
              </tbody>
            </table>
          </div>
