...
export const buildResultBlocks = (results: ToolResult[]) => {
  ...
  for (const result of results) {
    ...
    if (result.tool === 'getLowStockItems' && typeof result.data === 'object' && result.data) {
      const data = result.data as { items?: Array<{ name?: string; sku?: string; quantity?: number; lowStockAt?: number; categoryName?: string }> }
      blocks.push({
    ...
    if (result.tool === 'getExpiringItems' && typeof result.data === 'object' && result.data) {
      const data = result.data as { items?: Array<{ name?: string; sku?: string; quantity?: number; expiryDate?: string; category?: string }> }
      blocks.push({
    ...
    if (result.tool === 'searchItemBySkuOrName' && typeof result.data === 'object' && result.data) {
      const data = result.data as { items?: Array<{ name?: string; sku?: string; quantity?: number; availableQty?: number; category?: string }> }
      blocks.push({
    ...
    if (result.tool === 'getReorderSuggestions' && typeof result.data === 'object' && result.data) {
      const data = result.data as { suggestions?: Array<{ name?: string; sku?: string; currentQuantity?: number; suggestedReorderQty?: number; priority?: string }> }
      blocks.push({
